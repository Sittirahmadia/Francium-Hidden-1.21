...
public class GPFR {
	...
	private GP regularGlyphPage, boldGlyphPage, italicGlyphPage, boldItalicGlyphPage;
	

	public GPFR(GP regularGlyphPage, GP boldGlyphPage, GP italicGlyphPage,
                GP boldItalicGlyphPage) {
	...
	public static GPFR create(String fontName, int size, boolean bold, boolean italic,
                              boolean boldItalic) {
		...
		regularPage = new GP(new Font(fontName, Font.PLAIN, size), true, true);

		regularPage.generateGlyphPage(chars);
		regularPage.setupTexture();

		GP boldPage = regularPage;
		GP italicPage = regularPage;
		GP boldItalicPage = regularPage;

		if (bold) {
			boldPage = new GP(new Font(fontName, Font.BOLD, size), true, true);
		...
		if (italic) {
			italicPage = new GP(new Font(fontName, Font.ITALIC, size), true, true);
		...
		if (boldItalic) {
			boldItalicPage = new GP(new Font(fontName, Font.BOLD | Font.ITALIC, size), true, true);

			boldItalicPage.generateGlyphPage(chars);
			boldItalicPage.setupTexture();
		}

		return new GPFR(regularPage, boldPage, italicPage, boldItalicPage);
	}

	public static GPFR createFromID(String id, int size, boolean bold, boolean italic,
                                    boolean boldItalic) {
		char[] chars = new char[256];
		...
		try {
			font = Font.createFont(Font.TRUETYPE_FONT, GPFR.class.getResourceAsStream(id))
		...
		regularPage = new GP(font, true, true);
		regularPage.generateGlyphPage(chars);
		regularPage.setupTexture();
		...
		try {
			if (bold) {
				boldPage = new GP(
						Font.createFont(Font.TRUETYPE_FONT, GPFR.class.getResourceAsStream(id))
	...
	public int drawCenteredString(MatrixStack matrices, String text, double x, double y, float scale, int color) {
	...
	public int drawString(MatrixStack matrices, String text, float x, float y, float scale, int color, boolean dropShadow) {
		this.resetStyles();
		int i;
	...
	private void renderStringAtPos(MatrixStack matrices, String text, boolean shadow, int color) {
		GP glyphPage = getCurrentGlyphPage();
	...
	private void doDraw(float f, GP glyphPage) {
		if (this.strikethroughStyle) {
			Tessellator tessellator = Tessellator.getInstance();
			BufferBuilder bufferBuilder = tessellator.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION);
			bufferBuilder.vertex((float) this.posX, (float) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2)), 0.0F);
			bufferBuilder.vertex((float) (this.posX + f), (float) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2)), 0.0F);
			bufferBuilder.vertex((float) (this.posX + f), (float) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2) - 1.0F), 0.0F);
			bufferBuilder.vertex((float) this.posX, (float) (this.posY + (float) (glyphPage.getMaxFontHeight() / 2) - 1.0F), 0.0F);
		...
		if (this.underlineStyle) {
			...
			bufferBuilder.vertex((float) (this.posX + (float) l), (float) (this.posY + (float) glyphPage.getMaxFontHeight()), 0.0F);
			bufferBuilder.vertex((float) (this.posX + f), (float) (this.posY + (float) glyphPage.getMaxFontHeight()), 0.0F);
			bufferBuilder.vertex((float) (this.posX + f), (float) (this.posY + (float) glyphPage.getMaxFontHeight() - 1.0F), 0.0F);
			bufferBuilder.vertex((float) (this.posX + (float) l), (float) (this.posY + (float) glyphPage.getMaxFontHeight() - 1.0F), 0.0F);
			BufferRenderer.drawWithGlobalProgram(bufferBuilder.end());
		}
	...
	private GP getCurrentGlyphPage() {

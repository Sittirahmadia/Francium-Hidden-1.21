...
public enum ABU {;
    public static boolean isAnchorCharged(BlockPos anchor) {
        if (!isBlock(Blocks.RESPAWN_ANCHOR, anchor)) {
            ...
            try {
                return (Integer) BU.getBlockState(anchor).get(RespawnAnchorBlock.CHARGES) != 0;
    ...
    public static boolean canPlace(BlockState state, BlockPos pos) {
        return Client.MC.world.canPlace(state, pos, net.minecraft.block.ShapeContext.absent());
    }

    public static boolean hasBlock(BlockPos pos) {
        return !Client.MC.world.getBlockState(pos).isAir();
    }
    ...
    public static Block getBlock(BlockPos pos) {
        return Client.MC.world.getBlockState(pos).getBlock();
    }

    public static BlockState getBlockState(BlockPos pos) {
        return Client.MC.world.getBlockState(pos);
    }
    ...
    public static BlockHitResult clientRaycastBlock(BlockPos pos) {
        return Client.MC.world.raycastBlock(RoU.getEyesPos(), RoU.getClientLookVec().multiply(6.0D).add(RoU.getEyesPos()), pos, getBlockState(pos).getOutlineShape(Client.MC.world, pos), getBlockState(pos));
    }
    ...
    public static boolean isBlockReachable(BlockPos pos, double reach) {
        BlockState state = BU.getBlockState(pos);
        VoxelShape shape = state.getOutlineShape(Client.MC.world, pos);
        if (shape.isEmpty()) {
            shape = fullCube();
        }

        Vec3d eyesPos = RoU.getEyesPos();
    ...
    public static boolean rightClickBlock(BlockPos pos) {
        ...
        if (shape.isEmpty()) {
            ...
            ActionResult result2 = Client.MC.interactionManager.interactBlock(Client.MC.player, Hand.MAIN_HAND, new BlockHitResult(hitVecs[side.ordinal()], side, pos, false));

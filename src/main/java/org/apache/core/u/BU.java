...
//		}
		...
		if (!BU.canPlace(state, pos))
			return false;

		// if there is no clickable neighbors
		ArrayList<BlockPos> neighbors = BU.getClickableNeighbors(pos);
		...
		{
			BlockState block = BU.getBlockState(neighbor);
			...
			BlockHitResult hit = MC.world.raycastBlock(RoU.getEyesPos(), faceCenter, neighbor, BU.getBlockState(neighbor).getOutlineShape(MC.world, neighbor), BU.getBlockState(neighbor));
			if (hit == null)
			{
	...
	{
		return MC.world.canPlace(state, pos, net.minecraft.block.ShapeContext.absent());
	}
